...
test.beforeEach(`SalesForce lead Generation`, async ({ page }) => {
    ...
    searchname = await getResource(page.request, id)
    console.log(searchname)
})
test(`Salesforce using ui data verification`, async ({ page }) => {
    await page.goto("https://login.salesforce.com/")
    await page.fill("#username", "naveenkannan1501@gmail.com")
    await page.fill("#password", "Timesup@15")
    ...
    await page.waitForLoadState('load')
    await page.getByPlaceholder("Search this list...").fill(searchname, { timeout: 2000 })
})